...
import random
import pickle  #PARA SEREALIZACION DE ARCHIVOS
import os #Reccorido de arhvios
import string
...
'''Alta perimitiendo repetidos
Si fuerna sin permitir repetido seria algo asi:
Si se encuentra algún registro cuya clave coincida con este último, pero que además no esté 
marcado como eliminado, entonces la operación de alta se rechaza (pues ya existe un 
registro activo con esa clave). Si se llega al final del archivo y no se encontró repetida la 
clave, entonces se terminan de cargar por teclado los campos del registro a dar de alta, se 
asigna en True su campo de marcado lógico, y se graba el registro al final del archivo usando 
pickle.dump() (típicamente, el archivo debería estar abierto en modo 'a+b': es necesario 
poder leerlo para buscar el registro y saber si estaba repetido o no, y es necesario poder 
grabar el nuevo registro si fuese el caso, pero agregándolo al final).'''

def generar_archivo_en_binario(fd, n):
    m = open(fd, "ab")  #Intruccion con nombre de archivo y modo de sólo append para archivo binario (todas las grabaciones se  hacen al final del archivo, preservando su contenido previo si el archivo ya existía
    for i in range(n):
        clave = generar_clave() #Le genera una clave aleatoria
        nombre = generar_nombre() #le elegie un nombre aleatoriao de la lista
        descripcion = generar_descripcion() #le genera un nombre aleatorio
        cantidad = random.randint(0, 1500)
        tipo_deporte = random.randint(1, 50)
        nivel = random.randint(0, 9)
        pickle.dump(Programa(clave, nombre, descripcion, cantidad, tipo_deporte, nivel), m) #Graba los valores de las variables en el archivo
    m.close()  #SIEMPRE CERRAR EL ARCHVIO AL FINAL
    print("Archivo generado...")

# 2)
def mostrar_archivo(fd):
    hay_datos = False
    contador = 0 #Contador para mostrar la cantidad de registors que hay
    if os.path.exists(fd):
        m = open(fd, "rb") #abre el archivo binario solo en modo lectura
        size = os.path.getsize(fd)  #OBtiene el tamaño de bytes del archivo
        print("-" * 160)
        print(titulos())  #Armamos una linea titulos para visualizar los datos
        print("-" * 160)
        while m.tell() < size: #con tell reotrna el valor del filepointer como valor entero, entonces esto permite un recorrido secuencial del arhcivo
            hay_datos = True
            programa = pickle.load(m) #Leer el contenido del archivo
            contador += 1
            print(programa)
        m.close() #cerramos el archivo siempre al final
...
def add_in_order(vec, programa):
    ...
    '''la variable pos contiene el índice del paciente que debería quedar como el 
siguiente del que se quiere insertar (el primer paciente con número de historia clínica mayor 
al del paciente nuevo). Por lo tanto, el registro paciente debe agregarse justamente en la 
posición pos, corriendo hacia la derecha a todos los registros desde allí en adelante, lo cual 
sabemos que puede hacerse con la instrucción siguiente '''
...
def principal():
    fd = "Programas.dat" #Nombre archvo
    vec = []
    opcion = 0
    while opcion != 9:
        print("-" * 60)
        ...
        if opcion == 1:
            ...
            generar_archivo_en_binario(fd, n) #Le el nombre del archivo y la cant de objetos a crear
        elif opcion == 2:
...
'''No utilizo en ninguno momento seek pero Permite cambiar el valor del file pointer'''