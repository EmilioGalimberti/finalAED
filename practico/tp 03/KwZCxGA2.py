...
def consulta_por_genero(v):
    ...
    insertion_sort(v)
    for i in range(len(v)):
        if v[i].genero == genero[cont_g.index(may_g)]:
            res = '\t| Codigo: ' + str(v[i].codigo)
            res += ' | Titulo: ' + str(v[i].titulo)
            res += ' | Genero: ' + str(v[i].genero)
            res += ' | Idioma: ' + str(v[i].idioma)
            res += ' | Precio: $' + str("{:.2f}".format(v[i].precio * 1.1)) + '|\n'
            print(res)
...
def consulta_precio_por_grupo(v):
    ...
    for c in range(cant):
        ...
        if not flag_encontrado:
            print("\t(( Libro No encontrado ))")
    print('El precio de todos los libros solicitados y disponibles es de: $ ', round(acum, 2))
